...
D3D11_BUFFER_DESC StructuredBufferDesc(uint64_t count, bool uav = true, bool dynamic = false)
{
	...
	desc.CPUAccessFlags = 0;  //!dynamic ? 0 : D3D11_CPU_ACCESS_WRITE;
	desc.StructureByteStride = sizeof(T);
	desc.ByteWidth = (UINT)(sizeof(T) * count);
	return desc;
}
D3D11_BUFFER_DESC StructuredBufferDesc(uint64_t count, uint32_t size, bool uav = true, bool dynamic = false)
{
	D3D11_BUFFER_DESC desc{};
	desc.Usage = (uav || !dynamic) ? D3D11_USAGE_DEFAULT : D3D11_USAGE_DYNAMIC;
	desc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	...
	desc.ByteWidth = (UINT)(size * count);
	return desc;
}
//D3D11_BUFFER_DESC StructuredBufferDesc(uint64_t count, uint32_t size, bool uav = true, bool dynamic = false, bool staging = false)
//{
//	D3D11_BUFFER_DESC desc{};
//	desc.Usage = (uav || !dynamic) ? D3D11_USAGE_DEFAULT : D3D11_USAGE_DYNAMIC;
//	if (staging) {
...
//		desc.BindFlags = 0;
//	}
//	desc.MiscFlags = staging ? 0 : D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
//	desc.CPUAccessFlags = !dynamic ? 0 : D3D11_CPU_ACCESS_WRITE;
//	if (staging) {
//		desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
//	}
//	desc.StructureByteStride = staging ? 0 : size;
//	desc.ByteWidth = (UINT)(size * count);
//	return desc;
//}
